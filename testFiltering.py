...
class CollaborativeFiltering(object):
    ...
    def normalize_data(self, data, row_mean):
        ...
        if isinstance(data, pd.DataFrame) and isinstance(row_mean, pd.Series):
            if data.shape[0] != row_mean.shape[0]:
                print("The combination of parameters are incorrect!")
        elif isinstance(data, pd.Series) and isinstance(row_mean, float):
            pass
        else:
            print("The combination of parameters are incorrect!")

        normalizeData = data.subtract(row_mean,axis=0)
    ...
    def get_cosine_similarity(self, vector1, vector2):
        ...
        axis = vector1Axis or vector2Axis
        ...
        dotProductAB = (vector1 * vector2).sum(axis = axis)
        magnitudeProductAB = np.sqrt(np.square(vector1).sum(vector1Axis)) * np.sqrt(np.square(vector2).sum(vector2Axis))

        cosineSimilarity = dotProductAB/magnitudeProductAB
    ...
    def get_k_similar(self, data, vector):
        ...
        normalizeData = self.normalize_data(data,self.get_row_mean(data))
        normalizeVector = self.normalize_data(vector,self.get_row_mean(vector))
        ...
        cosineSimilarity = self.get_cosine_similarity(normalizeData,normalizeVector)
        ...
        kSimilarIndex = cosineSimilarity.nlargest(self.k)

        # TODO: Return 2 values. See function comment

        return [kSimilarIndex.index,kSimilarIndex]
    ...
    def get_rating(self, data, index, column):
        ...
        # class
        topKSimilarItems = self.get_k_similar(new_data,vector)
        # TODO: Compute for the rating using the similarity values and the raw
        # ratings for the k similar items.
        rawRatings = new_data.loc[topKSimilarItems[0]].iloc[:,column]

        numeratorRating = (rawRatings * topKSimilarItems[1]).sum()
        denomanatorRating = topKSimilarItems[1].sum()

        rating = numeratorRating/denomanatorRating
